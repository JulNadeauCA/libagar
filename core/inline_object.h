...
#endif
{
	...
	if ((V->info.pFlags & AG_VARIABLE_P_READONLY)) {
		AG_FatalErrorV("E30", "Object is const. "
		                      "Did you mean AG_CONST_OBJECT_NAMED()?");
	}
#endif
	return (V->data.p);
}
...
/* Accessor routine for AG_OBJECT_NAMED() macro in AG_Event(3). */
#ifdef AG_INLINE_HEADER
static __inline__ const void *_Nonnull _Pure_Attribute
AG_GetNamedConstObject(AG_Event *_Nonnull event, const char *_Nonnull key,
    const char *_Nonnull classSpec)
#else
const void *
ag_get_named_const_object(AG_Event *event, const char *key, const char *classSpec)
#endif
{
	AG_Variable *V;
...
#ifdef AG_TYPE_SAFETY
	if (!AG_OfClass((struct ag_object *)V->data.p, classSpec)) {
		AG_FatalError("Illegal AG_CONST_OBJECT_NAMED() access");
	}
	if (!(V->info.pFlags & AG_VARIABLE_P_READONLY)) {
		AG_FatalErrorV("E30", "Object is not const. "
		                      "Did you mean AG_OBJECT_NAMED()?");
	}
#endif
	return (V->data.p);
}
