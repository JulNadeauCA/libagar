...
if [ "$1" = "--libs" ]; then
	echo "-L${PREFIX}/lib -L/opt/local/lib -L/usr/local/lib -L/usr/X11R6/lib -lag_gui -lag_core -lag_net -lSDLmain -lSDL -Wl,-framework,Cocoa -framework OpenGL -lm -lpthread"
fi
