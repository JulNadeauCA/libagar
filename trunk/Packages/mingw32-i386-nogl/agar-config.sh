...
if [ "$1" = "--threads" ]; then echo "no"; fi
if [ "$1" = "--network" ]; then echo "no"; fi
if [ "$1" = "--sdl" ]; then echo "yes"; fi
if [ "$1" = "--opengl" ]; then echo "no"; fi
...
if [ "$1" = "--libs" ]; then
	echo "-L${PREFIX}/lib -lag_gui -lag_core -L/usr/local/lib -lmingw32 -lSDLmain -lSDL -mwindows -lfreetype -lm"
fi
